...
export default function ReportsPage() {
  ...
  const card = (title: string, children: React.ReactNode) => (
    <div className="card" style={{ marginBottom: 24 }}>
      <div className="card-header"><h3 style={{ margin: 0, fontSize: 14 }}>{title}</h3></div>
  ...
  return (
    <AppShell title="Отчёты" subtitle="Ежедневная сводка, черный список, стоп-слова, аналитика">
      {loading ? <p style={{ color: "var(--text-3)", padding: 24 }}>Загрузка...</p> : (
        <>
          {/* Summary pills */}
          <div style={{ display: "flex", gap: 12, marginBottom: 24, flexWrap: "wrap" }}>
            {[
              ...
              <div key={p.label} style={{ background: "var(--bg-2)", border: "1px solid var(--border)", borderRadius: 12, padding: "12px 20px", minWidth: 140 }}>
                <div style={{ fontSize: 22, fontWeight: 700, color: p.color }}>{p.value}</div>
                <div style={{ fontSize: 11, color: "var(--text-3)", marginTop: 2 }}>{p.label}</div>
              </div>
            ))}
          </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ borderBottom: "1px solid var(--border)", color: "var(--text-3)" }}>
                  {["Дата", "Новых", "Принято", "Отклонено", "Сохранено"].map(h => (
              ...
              <tbody>
                {daily.map(r => (
                  <tr key={r.day} style={{ borderBottom: "1px solid var(--border)" }}>
                    <td style={{ padding: "7px 12px" }}>{r.day}</td>
                    <td style={{ padding: "7px 12px", color: "var(--primary)", fontWeight: 600 }}>{r.new}</td>
                    <td style={{ padding: "7px 12px", color: "#22c55e" }}>{r.approved}</td>
                    <td style={{ padding: "7px 12px", color: "#ef4444" }}>{r.rejected}</td>
                    <td style={{ padding: "7px 12px", color: "var(--text-3)" }}>{r.saved}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, marginBottom: 24 }}>
            ...
            {card("👤 Активность менеджеров",
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                <thead>
                  <tr style={{ borderBottom: "1px solid var(--border)", color: "var(--text-3)" }}>
                    {["Менеджер", "Принял", "Откл.", "Изменил", "Сохр."].map(h => (
                ...
                <tbody>
                  {managers.map(r => (
                    <tr key={r.manager_id} style={{ borderBottom: "1px solid var(--border)" }}>
                      <td style={{ padding: "6px 8px", fontFamily: "monospace", fontSize: 11 }}>{r.manager_id}</td>
                      <td style={{ padding: "6px 8px", color: "#22c55e" }}>{r.approved}</td>
                      <td style={{ padding: "6px 8px", color: "#ef4444" }}>{r.rejected}</td>
                      <td style={{ padding: "6px 8px" }}>{r.edited}</td>
                      <td style={{ padding: "6px 8px", color: "var(--text-3)" }}>{r.saved}</td>
                    </tr>
                  ))}
          ...
          {card("⭐ Лучшие AI-ответы (принято без изменений)",
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {bestReplies.map(r => (
                <div key={r.id} style={{ background: "var(--bg-2)", borderRadius: 8, padding: 12, border: "1px solid var(--border)" }}>
          ...
          {card("⛔ Стоп-слова",
            <div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 16 }}>
                {stopWords.map(w => (
                  <span key={w} style={{ background: "var(--bg-3)", border: "1px solid var(--border)", borderRadius: 20, padding: "4px 12px", fontSize: 12, display: "flex", alignItems: "center", gap: 6 }}>
                    {w}
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <input value={newWord} onChange={e => setNewWord(e.target.value)}
                  ...
                  placeholder="Добавить слово..." className="input" style={{ flex: 1, maxWidth: 240 }} />
                <button onClick={addStopWord} className="btn btn-primary" style={{ whiteSpace: "nowrap" }}>Добавить</button>
              </div>
              ...
              <div style={{ marginBottom: 16 }}>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <input value={newPhone} onChange={e => setNewPhone(e.target.value)}
                    placeholder="+7 999 000-00-00" className="input" style={{ minWidth: 200 }} />
                  <input value={newReason} onChange={e => setNewReason(e.target.value)}
                    placeholder="Причина (необязательно)" className="input" style={{ flex: 1, minWidth: 200 }} />
                  <button onClick={addBlacklist} className="btn btn-primary">Добавить</button>
                </div>
              </div>
              {blacklist.length === 0 ? (
